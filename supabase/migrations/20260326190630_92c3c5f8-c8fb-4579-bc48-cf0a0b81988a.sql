...
-- Update check_apprenant_session: 30 min inactivity + 5 min grace
...
BEGIN
  ...
  v_presence_interval := interval '30 minutes';
  v_presence_due := v_conn.started_at + v_presence_interval;
  v_presence_deadline := v_presence_due + interval '5 minutes';
  v_max_end := v_conn.started_at + interval '7 hours';

  IF v_now >= v_max_end THEN
    UPDATE public.apprenant_connexions SET ended_at = v_max_end, last_seen_at = LEAST(COALESCE(last_seen_at, v_max_end), v_max_end), end_reason = 'max_duration', updated_at = now() WHERE id = v_conn.id AND ended_at IS NULL;
    RETURN QUERY SELECT false, 'max_duration'::text, false, 0, v_now, v_conn.started_at;
    RETURN;
  END IF;

  IF _event = 'confirm_presence' THEN
    UPDATE public.apprenant_connexions SET last_seen_at = v_now, updated_at = now() WHERE id = v_conn.id AND ended_at IS NULL;
  ...
  IF v_now >= v_presence_deadline AND v_conn.last_seen_at < v_presence_due THEN
    UPDATE public.apprenant_connexions SET ended_at = v_presence_deadline, last_seen_at = LEAST(COALESCE(last_seen_at, v_presence_due), v_presence_due), end_reason = 'no_response', updated_at = now() WHERE id = v_conn.id AND ended_at IS NULL;
    RETURN QUERY SELECT false, 'no_response'::text, false, 0, v_now, v_conn.started_at;
    RETURN;
  END IF;

  RETURN QUERY SELECT true, NULL::text, (v_now >= v_presence_due AND v_conn.last_seen_at < v_presence_due), GREATEST(0, FLOOR(EXTRACT(EPOCH FROM (v_presence_deadline - v_now)))::integer), v_now, v_conn.started_at;
END;
$function$;

-- Update enforce_apprenant_session_limits: 30 min + 5 min
...
BEGIN
  WITH closed_no_response_rows AS (
    UPDATE public.apprenant_connexions c SET ended_at = c.started_at + interval '35 minutes', last_seen_at = LEAST(COALESCE(c.last_seen_at, c.started_at + interval '30 minutes'), c.started_at + interval '30 minutes'), end_reason = 'no_response', updated_at = now()
    WHERE c.ended_at IS NULL AND now() >= c.started_at + interval '35 minutes' AND c.last_seen_at < c.started_at + interval '30 minutes'
    RETURNING 1
  )
  SELECT COUNT(*) INTO v_closed_no_response FROM closed_no_response_rows;

  WITH closed_max_rows AS (
    UPDATE public.apprenant_connexions c SET ended_at = c.started_at + interval '7 hours', last_seen_at = LEAST(COALESCE(c.last_seen_at, c.started_at + interval '7 hours'), c.started_at + interval '7 hours'), end_reason = 'max_duration', updated_at = now()
    WHERE c.ended_at IS NULL AND now() >= c.started_at + interval '7 hours'
    RETURNING 1
  )
...
-- Update is_current_user_apprenant: 30 min + 5 min window
...
AS $function$
  ...
  SELECT EXISTS (
    ...
    WHERE now() < c.started_at + interval '7 hours'
      AND (now() < c.started_at + interval '35 minutes' OR c.last_seen_at >= c.started_at + interval '30 minutes')
  );
$function$;
