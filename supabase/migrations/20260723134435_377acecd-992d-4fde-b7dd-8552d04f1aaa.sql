-- Fix ambiguity: RETURNS TABLE(ended_at ...) collides with column ended_at in UPDATE
...
BEGIN
  ...
  SET
    ended_at = v_end,
    last_seen_at = LEAST(COALESCE(c.last_seen_at, v_end), v_end),
    end_reason = v_reason,
    updated_at = now()
  WHERE c.id = v_conn.id
    AND c.ended_at IS NULL
  RETURNING c.ended_at INTO v_end;
