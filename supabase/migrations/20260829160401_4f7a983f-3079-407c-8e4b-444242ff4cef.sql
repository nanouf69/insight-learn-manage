-- Titre unique : ajout de l'heure (plusieurs devis possibles le même jour pour la même formation)
...
declare
  ...
  v_user_id uuid;
  v_date_txt text := to_char(coalesce(new.created_at, now()), 'DD/MM/YYYY HH24:MI');
  v_titre text;
begin
...
-- Rattrapage : devis sans trace liée
...
select d.apprenant_id,
       ...
       'Devis' || coalesce(' — ' || d.formation, '')
         || ' du ' || to_char(coalesce(d.created_at, now()), 'DD/MM/YYYY HH24:MI')
