CREATE TABLE public.examen_theorique_decalages (
  ...
  nouvelle_date text NOT NULL,
  ancienne_session_id uuid,
  nouvelle_session_id uuid,
  demandes_urgentes_closes integer NOT NULL DEFAULT 0,
  auteur uuid,
  auteur_email text,
  created_at timestamptz NOT NULL DEFAULT now()
);
GRANT SELECT ON public.examen_theorique_decalages TO authenticated;
GRANT ALL ON public.examen_theorique_decalages TO service_role;
ALTER TABLE public.examen_theorique_decalages ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Admins lisent les decalages" ON public.examen_theorique_decalages
  FOR SELECT TO authenticated USING (public.has_role(auth.uid(), 'admin'));

CREATE OR REPLACE FUNCTION public.examen_decalages_append_only()
RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
  RAISE EXCEPTION 'JOURNAL_DECALAGES_APPEND_ONLY' USING ERRCODE = 'P0495';
END $$;
CREATE TRIGGER trg_examen_decalages_append_only
  BEFORE UPDATE OR DELETE ON public.examen_theorique_decalages
  FOR EACH ROW EXECUTE FUNCTION public.examen_decalages_append_only();

CREATE OR REPLACE FUNCTION public.decaler_examen_theorique(
  p_operation_id uuid,
  p_apprenant_id uuid,
  p_ancienne_date text,
  p_nouvelle_date text,
  p_nouvelle_iso date,
  p_nouveau_lieu text DEFAULT NULL,
  p_email text DEFAULT NULL
) RETURNS public.examen_theorique_decalages
...
BEGIN
  ...
  -- Rejeu (double clic / F5) : on renvoie le resultat deja enregistre
  ...
  -- Rattachement aux sessions "examen" (deplacement, jamais de suppression)
  ...
  IF v_new_session IS NULL THEN
    ...
    VALUES ('Session examen — ' || p_nouvelle_date, p_nouvelle_iso, p_nouvelle_iso, p_nouveau_lieu, 'examen',
            'planifiee', 200, '{}', '[]'::jsonb, '14:00', '16:00')
  ...
  -- Demandes urgentes liees a l'ancienne session : closes (conservees)
  ...
  RETURN v_row;
END $$;
REVOKE ALL ON FUNCTION public.decaler_examen_theorique(uuid, uuid, text, text, date, text, text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.decaler_examen_theorique(uuid, uuid, text, text, date, text, text) TO authenticated;