...
function SlideSommaire({ slide, editing, onChange }: { slide: Slide & { type: "sommaire" }; editing: boolean; onChange?: (s: Slide) => void }) {
  ...
  return (
    <div className="p-6 bg-gradient-to-br from-blue-800 via-blue-700 to-blue-900 min-h-full text-white">
      ...
      <div className="space-y-2">
        {slide.items.map((it, i) => (
          <div key={i} className={`flex items-center gap-3 p-2 rounded-lg ${it.page === 0 ? 'bg-blue-900/50 mt-4' : 'hover:bg-blue-600/30'}`}>
            {editing ? (
              <>
                <EditableText value={it.n} onChange={v => updateItem(i, "n", v)} className="w-10 h-10 rounded-lg flex items-center justify-center font-bold text-sm bg-white/20 text-white text-center" />
              ...
              <>
                <span className={`flex-shrink-0 w-10 h-10 rounded-lg flex items-center justify-center font-bold text-sm ${it.page === 0 ? 'bg-blue-900 text-white' : 'bg-white/20 text-white'}`}>{it.n}</span>
                <span className={`text-sm font-medium flex-1 ${it.page === 0 ? 'text-white font-bold' : 'text-blue-100'}`}>{it.label}</span>
              </>
            )}
          </div>
        ))}
...
function EditableBlock({ block, onChange, onRemove }: { block: SlideBlock; onChange: (b: SlideBlock) => void; onRemove: () => void }) {
  ...
  return (
    <div className="rounded-lg border-l-4 bg-white/10 p-3 space-y-2" style={{ borderColor: block.color }}>
      ...
      {block.points.map((p, j) => (
        <div key={j} className="flex items-start gap-1">
           <span className="text-blue-200 text-xs mt-1">▸</span>
          <EditableText value={p} onChange={v => updatePoint(j, v)} className="text-xs text-blue-100 flex-1" />
          <button onClick={() => removePoint(j)} className="text-red-300/60 hover:text-red-200 p-0.5"><Trash2 className="w-3 h-3" /></button>
        </div>
      ))}
      <button onClick={addPoint} className="text-xs text-blue-200 hover:underline">+ Point</button>
    </div>
  );
}
...
function SlideContent({ slide, editing, onChange }: { slide: Slide & { type: "content" }; editing: boolean; onChange?: (s: Slide) => void }) {
  ...
  if (!editing) {
    return (
      <div className="p-5 space-y-4 bg-gradient-to-br from-blue-800 via-blue-700 to-blue-900 min-h-full text-white">
        <div>
          <h2 className="text-xl font-bold text-white">{slide.title}</h2>
          {slide.ref && <p className="text-xs text-blue-200 italic mt-1">{slide.ref}</p>}
        </div>
        {slide.intro && <p className="text-sm text-blue-100 leading-relaxed">{slide.intro}</p>}
        <div className="space-y-3">
          {slide.blocks?.map((b, i) => (
            <div key={i} className="rounded-lg border-l-4 bg-white/10 p-3" style={{ borderColor: b.color }}>
              <h3 className="font-bold text-sm mb-2" style={{ color: b.color }}>{b.heading}</h3>
              <ul className="space-y-1">
                {b.points.map((p, j) => (
                   <li key={j} className="text-xs text-blue-100 leading-relaxed pl-3 relative before:content-['▸'] before:absolute before:left-0 before:text-blue-300">{p}</li>
                ))}
              </ul>
            </div>
          ))}
  ...
  return (
    <div className="p-5 space-y-4 bg-gradient-to-br from-blue-800 via-blue-700 to-blue-900 min-h-full text-white">
      ...
      <EditableText value={slide.ref || ""} onChange={v => onChange?.({ ...slide, ref: v })} className="text-xs text-blue-200 italic" placeholder="Référence" />
      <EditableMultiline value={slide.intro || ""} onChange={v => onChange?.({ ...slide, intro: v })} className="text-sm text-blue-100" placeholder="Introduction" />
      <div className="space-y-3">
        {(slide.blocks || []).map((b, i) => (
      ...
      <EditableText value={slide.keyRule || ""} onChange={v => onChange?.({ ...slide, keyRule: v })} className="text-xs text-white bg-white/10 p-2 rounded" placeholder="Règle clé (optionnel)" />
    </div>
  );
}
...
function SlideTable({ slide, editing, onChange }: { slide: Slide & { type: "table" }; editing: boolean; onChange?: (s: Slide) => void }) {
  ...
  if (!editing) {
    return (
      <div className="p-5 space-y-4 bg-gradient-to-br from-blue-800 via-blue-700 to-blue-900 min-h-full text-white">
        <h2 className="text-xl font-bold text-white">{slide.title}</h2>
        {slide.intro && <p className="text-sm text-blue-100 mb-2">{slide.intro}</p>}
        <div className="overflow-x-auto">
          <table className="w-full text-xs border-collapse">
            <thead><tr>{slide.headers.map((h, i) => <th key={i} className="bg-blue-900 text-white p-2 text-left font-semibold">{h}</th>)}</tr></thead>
            <tbody>{slide.rows.map((row, i) => (
              <tr key={i} className={i % 2 === 0 ? "bg-white/10" : "bg-white/5"}>
                {row.map((cell, j) => <td key={j} className={`p-2 border-b border-white/10 ${j === 0 ? "font-semibold text-white" : "text-blue-100"}`}>{cell}</td>)}
              </tr>
            ))}</tbody>
          </table>
        </div>
        {slide.keyRule && <div className="bg-white/10 border border-white/20 rounded-lg p-3"><p className="text-xs font-semibold text-white">✔ {slide.keyRule}</p></div>}
        {slide.extraSections?.map((section, i) => (
          <div key={i} className="mt-3">
            <h3 className="text-sm font-bold text-white mb-2">{section.heading}</h3>
            <ul className="space-y-1">{section.items.map((item, j) => <li key={j} className="text-xs text-blue-100 pl-3 relative before:content-['▸'] before:absolute before:left-0 before:text-blue-300">{item}</li>)}</ul>
          </div>
        ))}
      </div>
    );
  ...
  return (
    <div className="p-5 space-y-4 bg-gradient-to-br from-blue-800 via-blue-700 to-blue-900 min-h-full text-white">
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-xs border-collapse">
          <thead><tr>
            {slide.headers.map((h, i) => <th key={i} className="bg-blue-900 p-1"><EditableText value={h} onChange={v => updateHeader(i, v)} className="text-white font-semibold text-xs" /></th>)}
            <th className="bg-blue-900 p-1"><button onClick={addCol} className="text-blue-200 text-xs">+Col</button></th>
          </tr></thead>
          <tbody>{slide.rows.map((row, i) => (
            <tr key={i} className={i % 2 === 0 ? "bg-white/10" : "bg-white/5"}>
              {row.map((cell, j) => <td key={j} className="p-1 border-b border-white/10"><EditableText value={cell} onChange={v => updateCell(i, j, v)} className="text-xs text-blue-100" /></td>)}
      ...
      <EditableText value={slide.keyRule || ""} onChange={v => onChange?.({ ...slide, keyRule: v })} className="text-xs text-white bg-white/10 p-2 rounded" placeholder="Règle clé" />
    </div>
  );
}
...
function SlideChiffres({ slide, editing, onChange }: { slide: Slide & { type: "chiffres" }; editing: boolean; onChange?: (s: Slide) => void }) {
  ...
  if (!editing) {
    return (
      <div className="p-5 bg-gradient-to-br from-blue-800 via-blue-700 to-blue-900 min-h-full text-white">
        <h2 className="text-xl font-bold text-white mb-4">{slide.title}</h2>
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
          {slide.items.map((it, i) => (
            <div key={i} className="bg-white/10 rounded-lg border border-white/20 p-3 text-center">
              <div className="text-lg font-bold text-white">{it.val}</div>
              <div className="text-xs text-blue-200 mt-1">{it.desc}</div>
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <div className="p-5 space-y-4 bg-gradient-to-br from-blue-800 via-blue-700 to-blue-900 min-h-full text-white">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
        {slide.items.map((it, i) => (
          <div key={i} className="bg-white/10 rounded-lg border border-white/20 p-3 text-center space-y-1 relative">
            <EditableText value={it.val} onChange={v => updateItem(i, "val", v)} className="text-lg font-bold text-white text-center" />
            <EditableText value={it.desc} onChange={v => updateItem(i, "desc", v)} className="text-xs text-blue-200 text-center" />
...
function SlideSchema({ slide, editing, onChange }: { slide: Slide & { type: "schema" }; editing: boolean; onChange?: (s: Slide) => void }) {
      ...
      <div className="p-5 space-y-4 bg-gradient-to-br from-blue-800 via-blue-700 to-blue-900 min-h-full text-white">
        ...
        {slide.tables?.map((table, i) => (
          <div key={i} className="overflow-x-auto">
            <table className="w-full text-xs border-collapse">
              <thead><tr>{table.headers.map((h, j) => <th key={j} className="bg-blue-900 text-white p-2 text-left font-semibold">{h}</th>)}</tr></thead>
              <tbody>{table.rows.map((row, ri) => (
                <tr key={ri} className={ri % 2 === 0 ? "bg-white/10" : "bg-white/5"}>
                  {row.map((cell, ci) => <td key={ci} className="p-2 border-b border-white/10 text-blue-100 text-xs">{cell}</td>)}
                </tr>
              ))}</tbody>
            </table>
          </div>
        ...
        {slide.lists?.map((list, i) => (
           <div key={i} className="rounded-lg border-l-4 border-blue-400 bg-white/10 p-3">
            <h3 className="font-bold text-sm text-white mb-2">{list.heading}</h3>
            <ul className="space-y-1">{list.items.map((item, j) => <li key={j} className="text-xs text-blue-100 pl-3 relative before:content-['▸'] before:absolute before:left-0 before:text-blue-300">{item}</li>)}</ul>
          </div>
        ))}
        {slide.keyRule && <div className="bg-white/10 border border-white/20 rounded-lg p-3"><p className="text-xs font-semibold text-white">✔ {slide.keyRule}</p></div>}
      </div>
  ...
  return (
    <div className="p-5 space-y-4 bg-gradient-to-br from-blue-800 via-blue-700 to-blue-900 min-h-full text-white">
      <EditableText value={slide.title} onChange={v => onChange?.({ ...slide, title: v })} className="text-xl font-bold text-white" />
      <EditableMultiline value={slide.intro || ""} onChange={v => onChange?.({ ...slide, intro: v })} className="text-sm text-blue-100" placeholder="Introduction" />
      <p className="text-xs text-blue-200 italic">Tableaux et listes (édition simplifiée)</p>
      {slide.lists?.map((list, i) => (
        <div key={i} className="rounded-lg border-l-4 border-blue-400 bg-white/10 p-3 space-y-1">
          ...
          {list.items.map((item, j) => (
            <div key={j} className="flex items-center gap-1">
              ...
              <EditableText value={item} onChange={v => {
                ...
                onChange?.({ ...slide, lists });
              }} className="text-xs text-blue-100 flex-1" />
            </div>
          ))}
        </div>
      ))}
...
function SlideSynthesis({ slide, editing, onChange }: { slide: Slide & { type: "synthesis" }; editing: boolean; onChange?: (s: Slide) => void }) {
      ...
      <div className="p-5 space-y-4 bg-gradient-to-br from-blue-800 via-blue-700 to-blue-900 min-h-full text-white">
        ...
        <div className="space-y-3">
          {slide.sections.map((s, i) => (
            <div key={i} className="rounded-lg border-l-4 bg-white/10 p-3" style={{ borderColor: s.color }}>
              <h3 className="font-bold text-sm mb-2" style={{ color: s.color }}>{s.heading}</h3>
              <ul className="space-y-1">{s.points.map((p, j) => <li key={j} className="text-xs text-blue-100 leading-relaxed pl-3 relative before:content-['▸'] before:absolute before:left-0 before:text-blue-300">{p}</li>)}</ul>
            </div>
          ))}
        </div>
        {slide.keyRule && <div className="bg-white/10 border border-white/20 rounded-lg p-3"><p className="text-xs font-semibold text-white">✔ {slide.keyRule}</p></div>}
  ...
  return (
    <div className="p-5 space-y-4 bg-gradient-to-br from-blue-800 via-blue-700 to-blue-900 min-h-full text-white">
      ...
      <div className="space-y-3">
        {slide.sections.map((s, i) => (
          <div key={i} className="rounded-lg border-l-4 bg-white/10 p-3 space-y-2" style={{ borderColor: s.color }}>
            <div className="flex items-center gap-2">
...
export default function SlideViewer({ slides, titre, brand, onBack, editable, onSlidesChange }: SlideViewerProps) {
  ...
  return (
    <div className="flex flex-col h-full min-h-[600px]">
      ...
      {/* Slide content */}
      <div className="flex-1 bg-blue-800 overflow-y-auto" style={{ minHeight: 420 }}>
        {renderSlide(slides[idx], editing, handleSlideChange)}
      </div>

      {/* Navigation */}
      <div className="bg-blue-900 px-4 py-3 flex items-center justify-between rounded-b-xl">
        <Button variant="outline" size="sm" onClick={prev} disabled={idx === 0} className="gap-1 border-white/30 text-white hover:bg-white/10">
        ...
        <Button size="sm" onClick={next} disabled={idx === total - 1} className="gap-1 bg-white text-blue-800 hover:bg-blue-100">
