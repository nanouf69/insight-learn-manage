                  ...
                  {allDocsFromFournisseurs.map((doc: any) => (
                    <div key={doc.id} className="flex flex-col gap-2 p-3 border rounded-lg bg-card hover:bg-muted/30 transition-colors">
                      ...
                      <div className="flex gap-2 mt-auto">
                        <Button variant="outline" size="sm" className="gap-1 flex-1" onClick={() => openDoc(doc.url)}><Eye className="w-3 h-3" />Voir</Button>
                        <Button variant="outline" size="sm" className="gap-1" onClick={() => downloadDoc(doc.url, doc.nom_fichier || doc.titre)}><FolderOpen className="w-3 h-3" /></Button>
                                    ...
                                    onClick={async () => {
                                      ...
                                      if (!to) return;
                                       try {
                                         ...
                                         const { data: sendData, error: sendErr } = await supabase.functions.invoke('send-document-email', {
                                           body: {
                                             recipientEmail: to,
                                             ...
                                             attachmentBucket: 'fournisseur-shared-docs',
                                             attachmentPath: doc.chemin_fichier || doc.file_path || null,
