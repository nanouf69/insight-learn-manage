...
export async function buildDossierApprenantIntoZip(
  ...
  try {
    ...
    try {
      ...
      for (const a of acts) if (a?.module_id != null && a?.module_nom && !moduleNameById.has(a.module_id)) moduleNameById.set(a.module_id, String(a.module_nom));
      ...
      const isAccueilLabel = (nom?: string | null) => !!nom && /accueil|liste\s+des\s+modules/i.test(nom);
      const enrichedRows = cnxRawRows.map((c: any) => {
        const startMs = c.started_at ? new Date(c.started_at).getTime() : NaN;
        const rawEndMs = (c.ended_at || c.last_seen_at) ? new Date(c.ended_at || c.last_seen_at).getTime() : NaN;
        if (!isFinite(startMs) || !isFinite(rawEndMs)) return { ...c, modules_consultes: [], quiz_realises: [], cours_exercices: [] };
        const endMs = Math.min(rawEndMs, startMs + MAX_SESSION_MS);
        ...
        for (const q of quizzes) {
          ...
          const scorePct = q.score_max ? ` (${Math.round((Number(q.score_obtenu) / Number(q.score_max)) * 100)}%)`
            : q.note_sur_20 != null ? ` (${Number(q.note_sur_20).toFixed(1)}/20)` : "";
